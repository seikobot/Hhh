...
// ─────────────────────────────────────────────
//  TOKEN — Lu depuis le Secret File Render (/etc/secrets/TOKEN)
//  ou depuis la variable d'environnement TOKEN
// ─────────────────────────────────────────────
const fs = require("fs");

function loadToken() {
  // 1. Secret File Render (recommandé)
  try {
    const secret = fs.readFileSync("/etc/secrets/TOKEN", "utf8").trim();
    if (secret) return secret;
  } catch {
    // Pas de secret file, on essaie la variable d'environnement
  }
...
const CONFIG = {
  // Token chargé depuis le Secret File Render (/etc/secrets/TOKEN)
  TOKEN: BOT_TOKEN,

  // ID de l'application Discord (variable d'environnement sur Render)
  CLIENT_ID: process.env.CLIENT_ID,

  // ID du serveur pour déployer les slash commands instantanément (variable d'environnement sur Render)
...
(async () => {
  try {
    ...
    if (CONFIG.GUILD_ID) {
      // Déploiement sur le serveur spécifique → visible instantanément
      await rest.put(
        Routes.applicationGuildCommands(CONFIG.CLIENT_ID, CONFIG.GUILD_ID),
        { body: slashCommands }
      );
    } else {
      // Fallback global si pas de GUILD_ID (peut prendre jusqu'à 1h)
      await rest.put(
